...
def apply_image_enhancement(image, enhancer):
    return np.array(enhancer.predict(image)) if isinstance(enhancer.predict(image), Image.Image) else enhancer.predict(image)
...
async def greet_user(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    keyboard = [
        ...
        [InlineKeyboardButton("Регулировка яркости", callback_data='brightness')],
    ...
    await update.message.reply_text(f"Привет! Я {BOT_USERNAME}, ваш AI-редактор изображений. Пожалуйста, выберите действие:", reply_markup=reply_markup)
...
async def process_image(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == 'enhance':
        ...
        grayscale_path = f'grayscale_image_{uuid.uuid4()}.jpg'
